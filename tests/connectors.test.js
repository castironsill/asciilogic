...
describe('binding math', () => {
  it('snaps a point to the nearest edge and records a fraction', () => {
    const b = box('s1', 0, 0, 100, 50);
    // Point near the right edge, vertically centered.
    const binding = makeBinding(b, 95, 25);
    expect(binding.id).toBe('s1');
    expect(binding.fracX).toBeCloseTo(1); // snapped to right edge
...
describe('Connectors integration', () => {
  ...
  it('binds an endpoint dropped on a shape and snaps it to the border', () => {
    ...
    expect(line.endX).toBe(100); // snapped onto right edge
  ...
  it('keeps a bound connector orthogonal via an auto bend', () => {
    const shape = box('s1', 0, 0, 40, 40);
    const line = { id: 'l1', type: 'line', startX: 200, startY: 100, endX: 40, endY: 20, endBinding: { id: 's1', fracX: 1, fracY: 0.5 } };
    const app = makeApp([shape, line]);
    const c = new Connectors(app);
    c.refresh();
    // endpoints are diagonal -> a bend was inserted
    expect(line.bendX).not.toBeUndefined();
